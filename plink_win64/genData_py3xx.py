...
import numpy as np
#np.set_printargs(precision=3, linewidth=200)
...
globalCovarCoeff = np.random.randn(1)[0] * args.covarStd
globalCovarCoeff = np.random.randn(1)[0] * args.covarStd
print( 'Covar coeff:', globalCovarCoeff)

bfile = args.bfile
bfile = args.bfile
numSnps = args.numSnps
numCausativeSNPs = args.csnps
numDiffSnps = args.numDiff
N = args.numIndividuals
casesFrac = args.caseFrac
hidCoeff = args.hidCoeff
numCases = int(N*casesFrac)
numControls = N - numCases
if (args.numSibs == 0): args.numSiblingsRatio = 0

fst = [args.fst, args.fst]
epsilon = [math.sqrt(1.0-args.her), math.sqrt(1.0-args.her)]
prevalence = args.prev
...
def generateIndividualPhenotype(individualMafCounts, numCausativeSNPs, snpEffectSizes, epsilon, freqs, covars):	
	...
	#epistasis handling
		...
		sumBeta2_np = snpEffectSizes[(2*args.numEpi):numCausativeSNPs].dot(snpEffectSizes[(2*args.numEpi):numCausativeSNPs])
		for i in range(0, 2*args.numEpi, 2):
			#dominant model			
...
def writeGenesFile(fileName, numSnps, numDiffSnps, numCausativeSNPs, chrom, posDiff):
	...
	maxDist = ((totalNumSnps-1)//args.geneSize)*posDiff
	pos = posDiff-10
	while True:
...
if (args.effStd == 0.0): snpEffectSizes = [1.0 for x in range(numCausativeSNPs)]
else: snpEffectSizes = [random.gauss(0.0, args.effStd) for x in range(numCausativeSNPs)]

#Effect size for hidden SNPs
try:
	for i in range(numCausativeSNPs-args.numHiddenSnps, numCausativeSNPs):		
		if (args.effStd > 0.0): snpEffectSizes[i] = random.gauss(0.0, args.effStd*hidCoeff)
		else: snpEffectSizes[i] = hidCoeff
except: pass

#Effect size for epistatic SNPs
for i in range(2*args.numEpi):			
	snpEffectSizes[i] = args.epiEff
...
ancestralFreqs = [random.random()*(0.5-args.minFreq) + args.minFreq for x in range(numSnps+numCausativeSNPs)]
for i in range(1,min(args.numRare+1, args.csnps-1)): ancestralFreqs[i] = args.rareFreq
...
for snpNum in range(numSnps+numCausativeSNPs):	
	...
	for popIndex in range(len(fst)):	
		snpFreq = 0.0
		if (fst[popIndex] < 1e-20 or (args.diffCause == 0 and snpNum < numCausativeSNPs-args.numHiddenSnps)): snpFreq = ancientFreq
		else:	#if SNP is hidden and causal, or non-causal
			while (snpFreq < args.minFreq or snpFreq > 1-args.minFreq):
				#if (snpNum >= 1 and snpNum < min(args.numRare+1, args.csnps-1)): break #rare variants should remain as is
...
for snpNum in range(numDiffSnps):	
	...
	freq2 = freq1 + 0.6
	if (args.diffSize >= 0):
		freq1 = 0.5 - args.diffSize
		freq2 = 0.5 + args.diffSize
...
numCaseCaseSiblings = int(args.numSiblingsRatio * numCases)
...
for popIndex in range(len(fst)):
	...
	controlIndex = 0
	#Generate siblings
	if (args.mateSibs==0 and (popIndex == siblingPop or True)):
		while (numCaseCaseSiblings > 0):
			(fatherSnps, motherSnps, sibSnps) = generateBrothers(args.numSibs, freqs, popIndex, numCausativeSNPs)
			for i in range(args.numSibs):
		...
		while (numCaseControlSiblings > 0 and controlIndex < len(popControlsSnps[popIndex])):
			(fatherSnps, motherSnps, sibSnps) = generateBrothers(args.numSibs, freqs, popIndex, numCausativeSNPs)
			for sibIndex in range(0, args.numSibs, 2):
...
#Generate mated sibs
if (args.mateSibs==1):
	numRemainingSibs = int(args.numSiblingsRatio * numCases * 3)
	while (numRemainingSibs > 0):
		...
		for childNum in range(args.numSibs):
...
createMapFile(numSnps, numDiffSnps, numCausativeSNPs, bfile+'.map', args.numHiddenSnps, args.numLD)
writeGenesFile(bfile+'.genes', numSnps, numDiffSnps, numCausativeSNPs, 1, 10*args.geneSize)
createPedFile(allLiabilities, allSnps, bfile+'.ped', numCausativeSNPs, args.numHiddenSnps, args.numLD)
...
createFrqFile(freqs, bfile+'.frq', args.csnps, args.numHiddenSnps)
createConfoundersFile(allSnps, allLiabilities, bfile+'.confounders', numCausativeSNPs, args.numHiddenSnps, popList)
